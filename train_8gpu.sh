...
#SBATCH -A ugk@gpu
#SBATCH --partition=gpu_p2s           # GPU partition requested
#SBATCH --ntasks-per-node 8
#SBATCH --nodes=1                   # number of nodes
#SBATCH --ntasks=8                    # number of processes (only one process here)
#SBATCH --gres=gpu:8                  # number of GPUs to reserve (only one GPU here)
#SBATCH --cpus-per-task=3            # number of cores to reserve (a quarter of the node)
